...
char **prune_word_list(char **words, int len, int *words_remaining) {
    int prune_index = 0;
    int buff_len = 1;
    char **pruned = malloc(buff_len * sizeof(char *));
    if (pruned == NULL) {
        perror("malloc"); 
        exit(1);
    }
    ...
    // iterate each word and store them in pruned with the specific len
    while(words[i] != NULL) {
        
        if (strlen(words[i]) == len) {
            if ( (pruned[prune_index] = malloc(strlen(words[i])+1 * sizeof(char))) == NULL) {
                perror("malloc"); 
                exit(1);
            }
            strncpy(pruned[prune_index], words[i], strlen(words[i])); 
            pruned[prune_index][strlen(words[i])] = '\0';

            if (prune_index + 1 == buff_len) {
                buff_len = buff_len*2;
                if ( (pruned = realloc(pruned, buff_len * sizeof(char*))) == NULL) {
                    perror("realloc"); 
                    exit(1);
                }
            }
    ...
    if (prune_index == 0) {
        *words_remaining = 0;
        ...
        pruned = realloc(pruned, (prune_index+1) * sizeof(char *));
        pruned[prune_index] = NULL;
    }

    *words_remaining = prune_index;
    return pruned;
}
...
char **get_word_list_of_length(char **words, int *len) {
    ...
    int *list_len = NULL;
    if ((list_len = malloc(sizeof(int))) == NULL) {
        perror("malloc");
        exit(1);
    }
    while(!valid) {
        if (fgets(line,BUF_SIZE, stdin)!=0) {
            valid = 1;
        }
        word_len = strtol(line, NULL, 10);
        
        words_list = prune_word_list(words, word_len, list_len);
        if(*list_len == 0) {
            printf("There are no words of that length.\n");
            printf("Length of words to use? ");
            valid = 0;
